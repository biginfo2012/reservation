<table class="table table-separate table-head-custom table-checkable" id="table">
    ...
    @foreach($data as $index => $item)
        <tr>
            ...
            <td class="p-0 border text-left align-middle px-1">{{isset($item['publish_time']) ? date('Y/m/d H:i', strtotime($item['publish_time'])) : ""}}</td>
            <td class="p-0 border text-left align-middle px-1">{{$item['title']}}</td>
            <td class="p-0 border text-left align-middle px-1">{{$item['status'] == 0 ? __('draft') : ($item['status'] == 1 ? __('stand') : __('published'))}}</td>
            <td class="p-0 border text-center align-middle">
                ...
                <button class="btn btn-outline-dark waves-effect ex_change delete-noti" style="padding: 8px; margin: 5px;" data-id="{{$item['id']}}">{{__('edit')}}</button>
            </td>
        </tr>
    @endforeach
    </tbody>
